...
- Доступность сервиса (uptime) - не менее 99.9% в рабочие часы (10:00-14:00)
...
![C4 Level 1 - Контекст](c4-level1.png)

---

## C4 Level 2 - Контейнеры

![C4 Level 2 - Контейнеры](c4-level2.png)
...
**React Native** - единый кодобаз для iOS и Android ускоряет запуск MVP: не нужны две отдельные команды и два релизных цикла.

**Python / FastAPI** - async-поддержка справляется с пиком заказов в обеденный час (10:00-14:00). Автодокументация OpenAPI сразу даёт готовую документацию API.

**PostgreSQL** - ACID-транзакции критичны: списание лимита и создание заказа должны быть атомарными, иначе возможен перерасход бюджета.

**Redis** - хранит OTP-коды с TTL 5 минут и текущие остатки лимитов. Снижает нагрузку на PostgreSQL в пиковые часы, когда сотни сотрудников одновременно проверяют баланс.

**ЮКасса** - Удобная платежная система с низким рейтинком падения

---

## Hiring Plan - Команда MVP

| Роль | Зоны ответственности |
|---|---|
| **Product Manager** (1) | Приоритизация бэклога, коммуникация с B2B-клиентами (HR-менеджеры, рестораны) |
| **Backend Developer** (1) | FastAPI-сервисы, логика лимитов и заказов, интеграция ЮКассы |
| **Frontend / Mobile Developer** (1) | React Native приложение для сотрудников, веб-панель HR и ресторана |
| **Designer** (1, part-time) | UX мобильного приложения, дизайн-система, прототипы для тестов |
| **QA Engineer** (1, part-time) | Тест-кейсы критического пути заказа, регресс перед релизами |
| **DevOps** (1, part-time) | CI/CD, деплой на VPS, мониторинг uptime в обеденные часы |

---

## Development Framework - Методология

- Обеденный пик (10:00-14:00) - жёсткое окно, в котором система не может падать. Scrum даёт чёткий ритм: каждые две недели выходит рабочий инкремент, который можно протестировать с реальными пользователями до следующего релиза.
- B2B-продажи идут параллельно разработке: PM каждый спринт переносит фидбек от HR-менеджеров и ресторанов в бэклог, а не копит его месяцами.
- Команда маленькая - Scrum без лишних церемоний (без отдельного Scrum Master, роль берёт PM). Ретроспектива раз в две недели не даёт техдолгу незаметно накапливаться.

---

## Team Rituals - Командные встречи

| Встреча | Цель | Частота |
|---|---|---|
| **Sprint Planning** | Выбрать задачи спринта, декомпозировать до конкретных тикетов | Раз в 2 недели, 1-1.5 ч |
| **Daily Standup** | Синхронизация: что сделано, что сегодня, есть ли блокеры | Каждый день, 15 мин |
| **Sprint Review** | Показать рабочий инкремент, собрать фидбек от стейкхолдеров | Раз в 2 недели, 30-45 мин |
| **Retrospective** | Что работает, что мешает, одно улучшение на следующий спринт | Раз в 2 недели, 30 мин |
| **Backlog Grooming** | Оценить и приоритизировать задачи перед следующим планингом | Раз в неделю, 30 мин |

---

## Риски проекта 

Стратегии реагирования определены по матрице «Влияние × Вероятность»:

| Влияние \ Вероятность | Низкая | Средняя | Высокая |
|---|---|---|---|
| Низкое | Не рассматривать | Принять | Принять |
| Среднее | Не рассматривать | Смягчить / Предотвратить | Смягчить / Предотвратить |
| Высокое | Передать (по возможности) | Смягчить / Предотвратить / Переделать план | Смягчить / Предотвратить / Переделать план |

---

| # | Риск | Тип | Вероятность | Влияние | Стратегия | Реагирование |
|---|---|---|---|---|---|---|
| R-01 | **Малое кол-во ресторанов на старте** - на старте мало партнёров, сотрудникам не из чего выбирать | Внешний | Высокая | Высокое | Предотвратить / Переделать план | Подписать 3-5 ресторанов до запуска без комиссии на 1-й месяц; закрытый пилот с одной компанией |
| R-02 | **HR-менеджеры не видят ценности** - клиент не готов платить, когда есть Яндекс.Еда | Внешний | Средняя | Высокое | Смягчить / Предотвратить | Бесплатный пилот 2 месяца с аналитикой расходов |
| R-03 | **Технический сбой в обеденный пик** - падение 10:00-14:00 | Внутренний | Низкая | Высокое | Передать / Смягчить | Uptime-мониторинг, нагрузочные тесты до релиза, резервное заказ по телефону или через поддержку в ТГ |
| R-04 | **Утечка платёжных данных** - потеря доверия, и юридические проблемы. | Внутренний | Низкая | Высокое | Передать / Смягчить | Не хранить карточные данные самостоятельно (всё через ЮКассу), HTTPS/TLS и т.д |
| R-05 | **Высокая стоимость привлечения B2B** - тяжело найти первых клиентов из BigTeh | Внешний | Средняя | Среднее | Смягчить / Предотвратить | Партнёрство с коворкингами и бизнес-центрами, уже после раскрутки и первых клиентов - сарафанное радио |
| R-06 | **Уход ключевого разработчика** - потеря одного из 3-4 останавливает разработку на недели | Внутренний | Средняя | Высокое | Смягчить / Предотвратить | Документация с первого дня, красивый и читаемый код, по взможности предложить более выгодные условия, чтобы сотрудник остался |
